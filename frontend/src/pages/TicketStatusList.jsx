...
export default function TicketStatusList() {
  ...
  const handleDelete = async (id) => {
    ...
    try { await statusesApi.delete(id); load(); } catch (err) { alert('Silme hatası: ' + err.message); }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-surface-0 rounded-xl border border-surface-200 shadow-xl w-full max-w-md mx-4">
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              <div><label className="block text-sm font-medium text-surface-700 mb-1.5">Ad <span className="text-danger">*</span></label><input type="text" required value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Durum adı..." className="input-field" /></div>
              <div className="flex items-center gap-2">
