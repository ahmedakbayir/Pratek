...
import { Plus, Edit3, Trash2, X, ShieldCheck } from 'lucide-react';
...
export default function PrivilegeList() {
  ...
  const load = () => {
    setLoading(true);
    privilegesApi.getAll().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  };
  ...
  const handleDelete = async (id) => {
    ...
    try { await privilegesApi.delete(id); load(); }
    catch (err) { alert('Silme hatası: ' + err.message); }
  };

  return (
    <div>
      <Header title="Yetkiler" subtitle={`${items.length} kayıt`} />
      <div className="p-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-surface-0 rounded-xl border border-surface-200 shadow-xl w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-surface-200">
              <h3 className="text-base font-semibold text-surface-900">{editing ? 'Yetki Düzenle' : 'Yeni Yetki'}</h3>
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              <div>
                ...
                <input type="text" required value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Yetki adı..." className="input-field" />
              </div>
              <div>
